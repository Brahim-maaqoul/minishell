...
/*   Updated: 2022/09/08 14:13:59 by bmaaqoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_red(char *str, int i)
{
	if (check_reder(str[i]))
	{
			...
			&& str[i + 2] == str[i + 3])
		{
			...
			|| (check_parenth(str[i + 2]) || check_parenth(str[i + 4])))
		{
			if (check_parenth(str[i + 2]))
				print_errs(str, 1, i + 2);
			else if (check_parenth(str[i + 4]))
				print_errs(str, 1, i + 4);
			else
				print_errs(str, 1, i + 2);
			return (0);
		}
